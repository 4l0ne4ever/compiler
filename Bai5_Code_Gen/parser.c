...
Type *compileLValue(void)
{
  ...
  case OBJ_PARAMETER:
    ...
    break;
  case OBJ_FUNCTION:
    // Function return value is at offset 0 in its frame
...
void compileCallSt(void)
{
  ...
  {
    compileArguments(proc->procAttrs->paramList);
    level = computeNestedLevel(symtab->currentScope, proc->procAttrs->scope->outer);
    genCALL(level, proc->procAttrs->codeAddress);
    genDCT(proc->procAttrs->paramCount);
  }
}
...
void compileIfSt(void)
{
  Instruction *fjInst;
  Instruction *jInst;
  CodeAddress labelElse, labelEndIf;
  ...
  if (lookAhead->tokenType == KW_ELSE)
  {
    ...
    labelEndIf = getCurrentCodeAddress();
    updateJ(jInst, labelEndIf);
  }
  else
  {
    labelEndIf = getCurrentCodeAddress();
    updateFJ(fjInst, labelEndIf);
  }
}

void compileWhileSt(void)
{
  CodeAddress labelWhile;
  Instruction *fjInst;
  CodeAddress labelEndWhile;

  labelWhile = getCurrentCodeAddress();
  ...
  genJ(labelWhile);
  labelEndWhile = getCurrentCodeAddress();
  updateFJ(fjInst, labelEndWhile);
}
...
void compileForSt(void)
{
  Type *varType;
  Type *type;
  CodeAddress beginLoop, labelEndFor;
  Instruction *fjInst;
  ...
  genJ(beginLoop);
  labelEndFor = getCurrentCodeAddress();
  updateFJ(fjInst, labelEndFor);
  genDCT(1);
}
...
Type *compileFactor(void)
{
  ...
  case TK_IDENT:
    ...
    case OBJ_FUNCTION:
      ...
      {
        compileArguments(obj->funcAttrs->paramList);
        level = computeNestedLevel(symtab->currentScope, obj->funcAttrs->scope->outer);
        genCALL(level, obj->funcAttrs->codeAddress);
        genDCT(obj->funcAttrs->paramCount);
      }
