...
Type *compileExpression3(Type *accumulatedType)
{
  Type *type;
  ...
  case SB_PLUS:
    ...
    type = compileTerm();
    checkIntType(type);
    checkIntType(accumulatedType);
    return compileExpression3(accumulatedType);
  case SB_MINUS:
    eat(SB_MINUS);
    type = compileTerm();
    checkIntType(type);
...
Type *compileTerm2(Type *accumulatedType)
{
  Type *type;
  ...
  case SB_TIMES:
    ...
    checkIntType(type);
    checkIntType(accumulatedType);
    return compileTerm2(accumulatedType);
  case SB_SLASH:
    eat(SB_SLASH);
    type = compileFactor();
    checkIntType(type);
