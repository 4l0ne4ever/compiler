...
ConstantValue *compileUnsignedConstant(void)
{
  ...
  case TK_IDENT:
    ...
    if (obj != NULL)
      ...
      constValue = makeIntConstant(0); // dummy value to continue parsing
...
ConstantValue *compileConstant2(void)
{
  ...
  case TK_IDENT:
    ...
    {
      constValue = makeIntConstant(1); // dummy value to continue parsing
    }
    break;
  default:
    error(ERR_INVALID_CONSTANT, lookAhead->lineNo, lookAhead->colNo);
...
Type *compileType(void)
{
  ...
  case TK_IDENT:
    ...
    if (obj != NULL)
      ...
      type = makeIntType(); // dummy type to continue parsing
    break;
  default:
...
void compileLValue(void)
{
  ...
  if (var != NULL)
  {
    ...
    case OBJ_PARAMETER:
      ...
      break;
    case OBJ_FUNCTION:
      /* assigning to a function name refers to its return value */
      break;
    default:
...
int compile(char *fileName)
{
  ...
  // Check if any semantic error occurred during parsing
  exitIfError();
